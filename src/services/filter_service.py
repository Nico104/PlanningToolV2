...
def filter_termine(
    ...
    if studiensemester:
        ...
        if lva_dict is None:
            raise ValueError(
                "Für den Studiensemester-Filter muss lva_dict (lva_id → LVA-Objekt) übergeben werden."
            )
        out = [
            t
            for t in out
    ...
    if studienrichtung:
        if lva_dict is None:
            raise ValueError(
                "Für den Studienrichtung-Filter muss lva_dict (lva_id → LVA-Objekt) übergeben werden."
            )
        out = [
            t
            for t in out
            if t.lva_id in lva_dict
            and getattr(lva_dict[t.lva_id], "studienrichtung", None) == studienrichtung
        ]
    if dozent:
        if lva_dict is None:
            raise ValueError(
                "Für den Dozent-Filter muss lva_dict (lva_id → LVA-Objekt) übergeben werden."
            )
        out = [
            t
            for t in out
            if t.lva_id in lva_dict
            and getattr(lva_dict[t.lva_id].vortragende, "name", None) == dozent
        ]
    if zu_besprechen:
