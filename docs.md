...
- Keep master data (LVA, rooms, generated semesters, etc.) consistent
...
### 2.3 Global Filters
- Studienrichtung
- Semester (Sommersemester/Wintersemester plus year)
- LVA
- Dozent
- Typ
- Raum
...
- Import dialog supports merging or overwriting existing data. Most entries are compared by ID and content; `freie_tage.json` uses `typ + beschreibung + von_datum + bis_datum` instead.

### 2.7 Semester Tools
- Available under `Tools -> Semester-Werkzeuge...`
- Copy Termine from one generated semester to another, grouped by LVA and selectable per LVA
- Default date transfer uses the same semester week and weekday in the target semester
- Alternate date transfer keeps the same calendar day in the target semester year
- Clear all Termine of a selected semester without deleting LVAs, rooms, Studiensemester, or other master data
...
- LVA can reference one or more Studiensemester and a Studienrichtung
- Free days can affect planning visibility and conflict logic
- Series Termine are stored as one Termin with `datum`, `datum_bis`, `periodizitaet`, and optional `ausfall_daten`
ID usage:
- IDs are used as primary references across JSON files for Termine, Räume, LVAs, and Studienrichtungen
- Free days are stored without technical IDs; their identity is `typ + beschreibung + von_datum + bis_datum`
- New technical IDs are generated by utility helpers where the data type still needs one
...
2. Fill required fields (name, LVA, Termin type, duration, etc.). The Termin type is selected from the allowed types of the chosen LVA.
3. Save
...
- Use Ctrl+Alt+T/L/R/F/G/H to open the matching Data Editor add dialog directly
...
### 6.11 Semester copy / clear

Copy flow:
1. Open `Tools -> Semester-Werkzeuge...`
2. Select source and target semester
3. Choose LVAs in the table
4. Select the date transfer mode
5. Confirm copy

The copy uses `semester_id` as the source criterion, not the calendar date range. This keeps exams or special Termine outside the usual semester range attached to their assigned semester. New Termine receive fresh IDs and keep room, LVA, type, group, duration, and series settings.
...
Only Termine with the selected `semester_id` are removed. Master data is left unchanged.

---

### 6.12 Data Path Behavior
