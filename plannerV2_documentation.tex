...
Daher geht es in dieser Arbeit um die Entwicklung eines Python-basierten Planungstools für die Unterstützung der LVA-Jahresplanung. Der Fokus liegt dabei auf der praktischen Anwendbarkeit dieser Software auf Fakultätsebene.
...
Die Arbeit beginnt in Kapitel 1 mit der Problemstellung, der Zielsetzung und der Forschungsfrage. Dadurch wird zuerst geklärt, warum ein Planungstool für die Lehrveranstaltungsplanung sinnvoll ist und welchen Beitrag diese Arbeit leisten soll.

Kapitel 2 beschreibt die fachlichen Grundlagen und den Kontext der Lehrveranstaltungsplanung. Dabei werden typische Planungsdaten, manuelle Arbeitsabläufe und die eingesetzten Technologien eingeordnet. In Kapitel 3 werden daraus die funktionalen und nicht-funktionalen Anforderungen an das Planungstool abgeleitet.

Kapitel 4 stellt die Konzeption des Planungstools vor. Dazu gehören die Grundidee der Anwendung, die Datenstruktur und der Aufbau der Benutzeroberfläche. Kapitel 5 beschreibt anschließend die Implementierung der zentralen Funktionen, zum Beispiel das Laden und Speichern von Projektdaten, die Kalenderansichten, Drag-and-Drop und die Konflikterkennung.

Kapitel 6 dokumentiert die Tests und die Evaluation des entwickelten Tools. In Kapitel 7 werden die Ergebnisse, Grenzen und möglichen Erweiterungen diskutiert. Kapitel 8 fasst die Arbeit abschließend zusammen und gibt einen Ausblick auf mögliche Weiterentwicklungen.

\clearpage

\section{Grundlagen und Kontext}

\subsection{Lehrveranstaltungsplanung auf Fakultätsebene}

Bei der Lehrveranstaltungsplanung geht es darum, die verschiedenen Lehrveranstaltungen einer Fakultät in einen passenden zeitlichen und organisatorischen Rahmen zu bringen. Dazu müssen Veranstaltungen wie Vorlesungen, Übungen, Labore oder Prüfungen konkreten Terminen, Räumen und teilweise auch bestimmten Gruppen zugeordnet werden. Die Planung ist daher mehr als nur das Eintragen von Terminen in einen Kalender.
...
Für eine Fakultät ist es wichtig, dass aus diesen Informationen ein übersichtlicher und nachvollziehbarer Plan entsteht.
...
Funktionale Anforderungen beschreiben, welche Aufgaben oder Dienste die Software erfüllen bzw. bereitstellen soll \cite{sommerville2016software}. Die wichtigsten funktionalen Anforderungen des Planungstools sind in Tabelle~\ref{tab:funktionale-anforderungen} dargestellt.
...
Die nicht-funktionalen Anforderungen des Planungstools sind in Tabelle~\ref{tab:nichtfunktionale-anforderungen} zusammengefasst.
...
Die grundlegende Projektstruktur ist in Abbildung~\ref{fig:projektstruktur} dargestellt.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{bilder/directory_structure.png}
    \caption{Vereinfachte Projektstruktur des Planungstools}
...
Listing~\ref{lst:hauptfenster-aufbau} zeigt vereinfacht, wie das Hauptfenster aufgebaut ist. Der Kalenderbereich wird als zentraler Arbeitsbereich gesetzt, während zusätzliche Funktionen wie die Terminliste über Dock-Bereiche eingebunden werden.

\Needspace{10\baselineskip}
\begin{lstlisting}[caption={Vereinfachter Aufbau des Hauptfensters}, label={lst:hauptfenster-aufbau}]
...
Die daraus entstehende Anordnung des zentralen Kalenderbereichs und der Dock-Bereiche entspricht der in Abbildung~\ref{fig:hauptfenster} gezeigten Benutzeroberfläche.
...
Listing~\ref{lst:konflikt-signal} zeigt eine solche Signal-Verbindung. Wird in der Konfliktanzeige ein Konflikt ausgewählt, wird das entsprechende Signal an den Kalender weitergegeben.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Signal-Verbindung zwischen Konfliktanzeige und Kalender}, label={lst:konflikt-signal}]
...
Eine weitere wichtige Aufgabe des Hauptfensters ist die Aktualisierung der Benutzeroberfläche. Wenn ein Termin erstellt, bearbeitet, gelöscht oder verschoben wird, müssen mehrere Bereiche den neuen Stand anzeigen. Dafür werden zentrale Aktualisierungsmethoden verwendet, wie Listing~\ref{lst:refresh-everything} zeigt.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Zentrale Aktualisierung der Benutzeroberfläche}, label={lst:refresh-everything}]
...
Die Termine werden in den Kalenderansichten als eigene Termin-Karten angezeigt. Dadurch können sie besser erkannt, ausgewählt und weiterverwendet werden. Je nach Terminart können die Karten unterschiedlich dargestellt werden. Der Aufbau einer Karte erfolgt dabei über die eigene \texttt{TerminCard}-Klasse, wie Listing~\ref{lst:termin-card} zeigt.

\Needspace{18\baselineskip}
\begin{lstlisting}[caption={Vereinfachter Aufbau einer Termin-Karte}, label={lst:termin-card}]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{bilder/data_editor.png}
...
Die Bearbeitung der Daten folgt dem CRUD-Prinzip (Erstellen, Anzeigen, Bearbeiten und Löschen). Im Projekt werden diese Aktionen nicht direkt an jeder Stelle neu umgesetzt, sondern über gemeinsame Funktionen im \texttt{CrudHandler} behandelt. Listing~\ref{lst:read-studiensemester} zeigt dazu ein Beispiel aus dem lesenden Teil der Datenverwaltung.

\Needspace{13\baselineskip}
\begin{lstlisting}[caption={Laden der Studiensemester-Modelle}, label={lst:read-studiensemester}]
...
Beim Drag-and-Drop wird nur die ID des jeweiligen Termins weitergegeben. Nach dem Ablegen kann die Anwendung anhand dieser ID den passenden Termin laden und die neuen Daten speichern. Listing~\ref{lst:drag-mimedata} zeigt die Übergabe der Termin-ID.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Übergabe der Termin-ID beim Drag-and-Drop}, label={lst:drag-mimedata}]
md = QMimeData()
...
Beim Ablegen wird die ID wieder ausgelesen, wie Listing~\ref{lst:drop-event} zeigt.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Auslesen der Termin-ID beim Ablegen}, label={lst:drop-event}]
...
Ein weiterer Teil der Drag-and-Drop Funktion ist die Vorschau während des Verschiebens. In der Tages- und Wochenansicht kann schon während des Ziehens geprüft werden, ob der Termin einen Konflikt auslösen würde. Dafür wird die neue Position nur vorübergehend simuliert. Listing~\ref{lst:dragdrop-konfliktpruefung} zeigt die vereinfachte Prüfung.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Prüfung der Drag-and-Drop-Konfliktvorschau}, label={lst:dragdrop-konfliktpruefung}]
...
Eine solche Konfliktvorschau während des Verschiebens zeigt Abbildung~\ref{fig:dragdrop-konflikt}.
...
Die Filter werden im \texttt{GlobalFilterDock} ausgewählt. Dieser Zustand wird als \texttt{FilterState} gespeichert und enthält alle aktuell gesetzten Filter. Listing~\ref{lst:filter-state} zeigt den vereinfachten Aufbau dieses Zustands.

\Needspace{14\baselineskip}
\begin{lstlisting}[caption={Filterzustand der globalen Filter}, label={lst:filter-state}]
class FilterState:
...
Wenn der Nutzer dort einen Filter ändert, wird diese Änderung an das Hauptfenster weitergegeben. Dafür wird die in Listing~\ref{lst:filter-signal} gezeigte Signalverbindung verwendet.

\Needspace{7\baselineskip}
\begin{lstlisting}[caption={Signalverbindung der globalen Filter}, label={lst:filter-signal}]
...
Die Filter ändern nicht die gespeicherten Termindaten. Stattdessen wird nur berechnet, welche Termine zu den aktuell gesetzten Filtern passen. Diese gefilterten Termine werden an die Terminliste übergeben, während der Kalender die Ansicht anhand des gespeicherten \texttt{FilterState} aktualisiert. Listing~\ref{lst:filter-update} zeigt diesen Ablauf vereinfacht.

\Needspace{10\baselineskip}
\begin{lstlisting}[caption={Aktualisierung nach geänderten Filtern}, label={lst:filter-update}]
...
Listing~\ref{lst:lecturer-conflicts} zeigt vereinfacht die Erkennung von Konflikten bei Lehrpersonen. Dafür werden Termine zuerst nach Lehrperson und Datum gruppiert. Anschließend wird jeder Termin innerhalb einer Gruppe mit jedem folgenden Termin verglichen, um zeitliche Überschneidungen zu finden.
%detect snippet
\Needspace{18\baselineskip}
\begin{lstlisting}[caption={Vereinfachte Erkennung von Lehrpersonen-Konflikten}, label={lst:lecturer-conflicts}]
...
Die eigentliche Prüfung wird über die Klasse \texttt{ConflictDetector} durchgeführt. Listing~\ref{lst:conflict-rule} zeigt eine einzelne Regel aus der Konfigurationsdatei.

\Needspace{9\baselineskip}
\begin{lstlisting}[caption={Beispiel einer Konfliktregel}, label={lst:conflict-rule}]
{
    "name": "Raum-Konflikt",
    "key": "room_conflict",
    "enabled": true,
...
\end{lstlisting}
Über \texttt{key} wird die Regel in der Anwendung einer konkreten Prüffunktion zugeordnet.
...
Beim Anklicken einer Konfliktkarte werden die betroffenen Termin-IDs gesendet. Diese IDs dienen als eindeutige Referenz auf die Termine, die im Kalender markiert werden sollen. Listing~\ref{lst:konfliktkarte-signal} zeigt die Weitergabe dieser IDs.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Weitergabe ausgewählter Konflikt-Termin-IDs}, label={lst:konfliktkarte-signal}]
def _on_card_clicked(self, termin_ids):
    if termin_ids:
        self.conflict_items_highlight.emit(termin_ids)
\end{lstlisting}

In der Kalenderansicht wird zuerst zum ersten betroffenen Termin gesprungen. Danach werden alte Markierungen entfernt, damit nur der aktuell ausgewählte Konflikt sichtbar bleibt. Anschließend werden die betroffenen Termine in der Tages-, Wochen- und Monatsansicht markiert. Listing~\ref{lst:highlight-termine} zeigt diesen Ablauf.

\Needspace{12\baselineskip}
\begin{lstlisting}[caption={Hervorhebung betroffener Termine im Kalender}, label={lst:highlight-termine}]
...
Abbildung~\ref{fig:export-lehrende} zeigt den Exportdialog für Lehrende. In diesem Dialog kann der Nutzer festlegen, welche LVAs bzw. Lehrpersonen exportiert werden sollen und ob die Termine als Tabellenliste oder als Wochenkalender ausgegeben werden.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{bilder/export_lehrende.png}
    \caption{Exportdialog für Lehrende mit Auswahl von LVAs, Zeitraum, Exportformat und Semestern}
    \label{fig:export-lehrende}
\end{figure}
...
Die Undo- und Redo-Funktion arbeitet mit gespeicherten Zwischenständen der Daten. Bevor eine Änderung durchgeführt wird, wird der aktuelle Zustand als Kopie aller Daten gespeichert. Wenn der Benutzer die Änderung rückgängig macht, wird der frühere Zustand wieder geladen und die Benutzeroberfläche aktualisiert. Listing~\ref{lst:undo-restore} zeigt die Wiederherstellung eines solchen Zustands.

\Needspace{8\baselineskip}
\begin{lstlisting}[caption={Wiederherstellung eines gespeicherten Zustands}, label={lst:undo-restore}]
...
Listing~\ref{lst:export-shortcut} zeigt die Zuweisung eines Shortcuts für die Exportfunktion:

\Needspace{6\baselineskip}
\begin{lstlisting}[caption={Shortcut für den Datenexport}, label={lst:export-shortcut}]
...
Die Tabellen~\ref{tab:test-stammdaten} bis~\ref{tab:test-speicherung} zeigen eine Auswahl der durchgeführten manuellen Tests, gegliedert nach Funktionsbereichen.
...
\label{tab:test-kalender-dragdrop}
\begin{tabularx}{\textwidth}{|L{3cm}|Y|Y|}
\hline
\textbf{Testfall} & \textbf{Durchführung} & \textbf{Ergebnis} \\
\hline
...
\hline
Filter zurücksetzen & Ein gesetzter Filter wird entfernt bzw. alle Filter werden gemeinsam über den \textit{Zurücksetzen}-Button zurückgesetzt. & Die zuvor ausgeblendeten Termine werden wieder angezeigt. \\
\hline
...
\hline
Kapazitäts\-warnung erzeugen & Ein Termin wird in einen Raum gelegt, dessen Kapazität für die Gruppengröße nicht ausreicht. & Die Kapazitätswarnung wird in der Konfliktanzeige angezeigt. \\
\hline
...
Damit lässt sich die Forschungsfrage beantworten: Ein solches Planungstool kann durch eine strukturierte Verwaltung der Daten, eine übersichtliche Benutzeroberfläche und eine regelbasierte Prüfung von Konflikten umgesetzt werden. Die manuellen Tests zeigen, dass zentrale Arbeitsabläufe wie das Anlegen, Bearbeiten, Verschieben, Filtern, Importieren und Exportieren von Terminen im entwickelten Prototyp funktionieren.
...
Dieser Anhang fasst zwei typische Einstiegsszenarien kurz zusammen. 

\subsection{Neues Projekt anlegen und eine LVA mit Termin hinzufügen}
\label{sec:howto-neues-projekt}

Dieses Beispiel beschreibt den Ablauf, wenn noch keine Projektdaten vorhanden sind und ein neues Projekt mit einer ersten Lehrveranstaltung und einem ersten Termin begonnen werden soll.

\begin{enumerate}
\item Anwendung starten.
\item Wenn noch kein Projektordner gespeichert ist, erscheint der Dialog \textit{Projekt auswählen}. Dort \textit{Neues Projekt anlegen} auswählen. Wenn bereits ein Projekt geöffnet ist, kann derselbe Ablauf über \textit{Datei} -- \textit{Neues Projekt} gestartet werden.
\item Einen leeren Ordner als Projektordner auswählen und das Projekt anlegen. In diesem Ordner erzeugt die Anwendung die benötigten JSON-Dateien für Termine, Lehrveranstaltungen, Räume, Studienrichtungen und freie Zeiträume.
\item Falls gewünscht, die angebotenen Standarddaten übernehmen. Dadurch können die mitgelieferten Raum- und LVA-Daten als Ausgangspunkt verwendet werden. Alternativ kann das Projekt ohne Standarddaten gestartet werden.
\item Eine neue Lehrveranstaltung im Dateneditor anlegen oder beim Erstellen des ersten Termins ergänzen. Dafür werden zumindest LVA-Nummer, Name und Studienrichtung eingetragen.
\item Einen neuen Termin über \textit{Datei} -- \textit{Neuer Termin} oder über den Dateneditor erstellen.
\item Die zuvor angelegte Lehrveranstaltung auswählen und die benötigten Termindaten eintragen, insbesondere Semester, Terminart und bei Bedarf Datum, Uhrzeit, Dauer und Raum.
\item Den Termin speichern. Danach erscheint er je nach Vollständigkeit entweder direkt in der Kalenderansicht oder in der Terminliste.
\item Falls der Termin noch nicht vollständig eingeplant ist, kann er per Drag-and-Drop aus der Terminliste in die Kalenderansicht gezogen werden.
\item Nach dem Speichern bzw. Verschieben werden Kalender, Terminliste und Konfliktanzeige aktualisiert. Mögliche Konflikte können anschließend über die Konfliktanzeige kontrolliert werden.
\end{enumerate}

\subsection{Beispielprojekt laden}

Dieses Beispiel beschreibt den Einstieg mit vorbereiteten Projektdaten aus der Datei \texttt{BeispielProjektDaten.xlsx}. Dafür wird zuerst ein neues Projekt angelegt und anschließend die Excel-Datei importiert.

\begin{enumerate}
\item Zunächst die Schritte~1 bis~4 aus Abschnitt~\ref{sec:howto-neues-projekt} durchführen. Dadurch wird ein neues Projekt in einem leeren Projektordner vorbereitet; die Standarddaten können übernommen oder übersprungen werden.
\item Nach dem Anlegen des Projekts im Menü \textit{Datei} -- \textit{Importieren} den Eintrag \textit{Importieren} auswählen.
\item Die Datei \texttt{BeispielProjektDaten.xlsx} auswählen.
\item Die Importübersicht prüfen. Sie zeigt je Datenbereich, wie viele Einträge insgesamt erkannt wurden und wie viele davon neu, geändert, bereits vorhanden oder übersprungen sind.
\item Bei geänderten bestehenden Einträgen öffnet sich ein Merge-Dialog. Dort werden die betroffenen Felder gegenübergestellt, sodass entschieden werden kann, ob der geänderte Eintrag übernommen oder ignoriert wird. Neue Einträge können ohne diesen Vergleich importiert werden.
\item Nach dem Import prüfen, ob Kalenderansicht, Terminliste und Dateneditor die importierten Daten anzeigen.
\item Über die Filter kann kontrolliert werden, ob bestimmte LVAs, Räume, Semester oder Lehrpersonen korrekt übernommen wurden.
\item Die Konfliktanzeige kann anschließend verwendet werden, um mögliche Überschneidungen oder Warnungen im importierten Planungsstand zu kontrollieren.
\end{enumerate}

\clearpage

\section{Optionsübersicht}

Dieser Anhang gibt eine kurze Übersicht über Optionen, die in der Anwendung an mehreren Stellen auswählbar oder konfigurierbar sind. Die konkreten Auswahlwerte, zum Beispiel verfügbare LVAs oder Räume, hängen vom jeweils geöffneten Projekt ab.

\subsection{Filter- und Suchoptionen}

\begin{itemize}
\item \textbf{Semester:} Einschränkung auf ein bestimmtes Sommer- oder Wintersemester.
\item \textbf{Studienrichtung:} Einschränkung auf die einer LVA zugeordnete Studienrichtung.
\item \textbf{Studiensemester:} Einschränkung auf die Studiensemester-Zuordnung der LVA.
\item \textbf{LVA:} Einschränkung auf eine konkrete Lehrveranstaltung.
\item \textbf{Dozent:} Einschränkung auf die hinterlegte Lehrperson.
\item \textbf{Typ:} Einschränkung auf Veranstaltungstypen wie VO, UE, LU, PR, SE oder VU.
\item \textbf{Gebäude:} Einschränkung der Raumliste auf ein Gebäude.
\item \textbf{Raum:} Einschränkung auf einen konkreten Raum.
\item \textbf{Zu besprechen:} Anzeige von Terminen, die als zu besprechen markiert sind.
\item \textbf{Suche in der Terminliste:} Suche nach Terminname, Termin-ID, LVA, Raum, Lehrperson oder Besprechungshinweis.
\end{itemize}

\subsection{Konfliktregeln}

\begin{itemize}
\item \textbf{Raum-Konflikt:} Zwei Termine überschneiden sich im selben Raum.
\item \textbf{Gruppen-Konflikt:} Zwei Termine derselben LVA und Gruppe überschneiden sich.
\item \textbf{Lehrpersonen-Konflikt:} Zwei Termine mit derselben Lehrperson überschneiden sich.
\item \textbf{Studienplan-Warnung:} Zwei Termine derselben Studienrichtung und desselben Studiensemesters überschneiden sich.
\item \textbf{Feiertags-Konflikt:} Ein Termin liegt auf einem Feiertag.
\item \textbf{Vorlesungsfrei-Konflikt:} Ein Termin liegt in einem vorlesungsfreien Zeitraum.
\item \textbf{Unvollständiger Termin:} Ein Termin enthält fehlende Angaben für Datum, Startzeit, Dauer oder Raum.
\item \textbf{Kapazitätswarnung Übung/Labor/Projekt:} Für UE, LU, PR und SE wird die Raumkapazität mit der Gruppengröße verglichen.
\item \textbf{Kapazitätswarnung Vorlesung/VU:} Für VO und VU wird die Raumkapazität mit der Gruppengröße verglichen.
\item \textbf{Dauerwarnung:} Die Termindauer liegt außerhalb des eingestellten Minutenbereichs.
\item \textbf{Startzeit-Warnung:} Ein Termin beginnt nicht zu einer vollen Stunde.
\item \textbf{Samstagswarnung:} Ein Termin liegt auf einem Samstag.
\item \textbf{Sonntagswarnung:} Ein Termin liegt auf einem Sonntag.
\end{itemize}

\subsection{Import- und Exportoptionen}

\begin{itemize}
\item \textbf{Projektimport:} Einlesen von JSON-, Excel- oder CSV-Dateien.
\item \textbf{Standarddatenimport:} Übernahme der mitgelieferten Raum- und LVA-Daten in ein Projekt.
\item \textbf{Import freier Tage:} Übernahme freier Zeiträume aus externen Quellen oder manuelle Ergänzung im Dialog.
\item \textbf{Gesamter Projektexport:} Export aller Projektbereiche als JSON- oder Excel-Datei.
\item \textbf{Export einzelner Projektbereiche:} Export einzelner Dateien wie Räume, LVAs, Termine, Studienrichtungen oder freie Zeiträume als JSON, Excel oder CSV.
\item \textbf{Export für Lehrende:} Export ausgewählter LVAs bzw. Lehrpersonen und Semester als Excel-Terminliste oder Wochenkalender.
\end{itemize}

\subsection{Semester-Werkzeuge}

\begin{itemize}
\item Termine eines Quellsemesters in ein Zielsemester kopieren.
\item Termine eines ausgewählten Semesters löschen.
\item Datum beim Kopieren nach Position im Semester oder nach gleichem Kalendertag übertragen.
\item Ausfalltermine und einzelne Serien-Ausnahmen übernehmen.
\item Serienvorkommen auf freien Tagen im Zielsemester automatisch als Ausfall markieren.
\end{itemize}

\subsection{Tastenkombinationen}

Die standardmäßig verfügbaren Tastenkombinationen der Anwendung sind in Tabelle~\ref{tab:shortcut-overview} zusammengefasst.

\begin{table}[H]
\centering
\small
\caption{Standard-Shortcuts der Anwendung}
\label{tab:shortcut-overview}
\begin{tabularx}{\textwidth}{|L{3.3cm}|Y|}
\hline
\textbf{Shortcut} & \textbf{Funktion} \\
\hline
\texttt{F5} & Ansicht aktualisieren \\
\hline
\texttt{Alt+Left} / \texttt{Alt+Right} & Zum vorherigen bzw. nächsten Zeitraum wechseln \\
\hline
\texttt{Ctrl+1} / \texttt{Ctrl+2} / \texttt{Ctrl+3} & Wochen-, Tages- bzw. Monatsansicht auswählen \\
\hline
\texttt{Ctrl+T} & Zum heutigen Datum springen \\
\hline
\texttt{Ctrl+N} & Neuen Termin erstellen \\
\hline
\texttt{Ctrl+I} & Importieren öffnen \\
\hline
\texttt{Ctrl+E} & Datenexport öffnen \\
\hline
\texttt{Ctrl+Shift+S} & Einstellungen öffnen \\
\hline
\texttt{Ctrl+Shift+K} & Konflikteinstellungen öffnen \\
\hline
\texttt{Ctrl+Shift+R} & Layouts zurücksetzen \\
\hline
\texttt{Ctrl+Z} / \texttt{Ctrl+Y} & Rückgängig bzw. Wiederholen \\
\hline
\texttt{Ctrl+Alt+T} & Neuen Termin erstellen \\
\hline
\texttt{Ctrl+Alt+L} & Neue LVA erstellen \\
\hline
\texttt{Ctrl+Alt+R} & Neuen Raum erstellen \\
\hline
\texttt{Ctrl+Alt+F} & Neuen freien Zeitraum erstellen \\
\hline
\texttt{Ctrl+Alt+H} & Neue Studienrichtung erstellen \\
\hline
\texttt{Ctrl+Alt+V} & Vorjahresansicht anzeigen bzw. umschalten \\
\hline
\texttt{Delete} / \texttt{Backspace} & Fokussierten Kalendertermin aus dem Kalender entfernen \\
\hline
\texttt{Ctrl+Delete} / \texttt{Ctrl+Backspace} & Fokussierten Kalendertermin löschen \\
\hline
\end{tabularx}
\end{table}
...
\url{https://csrc.nist.gov/glossary/term/usability}
